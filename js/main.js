...
/* WEBPACK VAR INJECTION */(function($) {$(function () {
  var arSideMenus = document.querySelectorAll('.sidebar-menu');

  if (arSideMenus) {
    arSideMenus.forEach(function (sideMenu) {
      var arSubMenus = sideMenu.querySelectorAll('.submenu');

      if (arSubMenus) {
        arSubMenus.forEach(function (subMenu) {
          var i = document.createElement("i"),
              parent = subMenu.parentNode,
              $submenu = $(subMenu);
          parent.insertBefore(i, subMenu);
          ...
          if ($(window).width() < 768) {
            var $item = $submenu.parent(),
                $link = $item.children().first(),
                $tempSelect = $('<select><option value="' + ($link.attr('href') ? $link.attr('href') : '') + '">' + $link.text() + '</option></select>');
...
$(function () {
  ...
  var swiperSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](videoSlider.get(0), {
    slidesPerView: 'auto',
  ...
  var videoFrame = $('.js-modal-show'),
      videoModal = $('.js-video-modal'),
      videoModalBtn = $('.video-modal-btn'),
      videoModalPrev = $('.js-video-modal-prev'),
      videoModalNext = $('.js-video-modal-next'),
      embed;

  function videoButtons(frame) {
    if (frame.parents('.video-slider').find('.is-active:first-child').length != 0) {
      videoModalBtn.removeClass('is-disable');
      videoModalPrev.addClass('is-disable');
    } else if (frame.parents('.video-slider').find('.is-active:last-child').length != 0) {
      videoModalBtn.removeClass('is-disable');
      videoModalNext.addClass('is-disable');
    } else {
      videoModalBtn.removeClass('is-disable');
    }
  }

  function videoEmbed(video, img) {
    if (video.length != 0) {
      embed = video.attr('src');
      videoModal.find('iframe').remove();
      videoModal.html('<iframe src=' + embed + '?autoplay=1 frameborder="0" allowfullscreen="1"></iframe>');
    } else {
      embed = img.attr('src');
      videoModal.find('img').remove();
      videoModal.html('<img src=' + embed + '>');
    }
  }

  function videoPrev(frame) {
    videoModalPrev.off('click');
    videoModalPrev.on('click', function () {
      videoEmbed(frame.parents('.video-slider').find('.is-active').prev().find('iframe'), frame.parents('.video-slider').find('.is-active').prev().find('img'));
      frame.parents('.video-slider').find('.is-active').prev().addClass('is-active');
      frame.parents('.video-slider').find('.is-active').next().removeClass('is-active');
      swiperSlider.slidePrev();
      videoButtons(frame);
    });
  }

  function videoNext(frame) {
    videoModalNext.off('click');
    videoModalNext.on('click', function () {
      videoEmbed(frame.parents('.video-slider').find('.is-active').next().find('iframe'), frame.parents('.video-slider').find('.is-active').next().find('img'));
      frame.parents('.video-slider').find('.is-active').next().addClass('is-active');
      frame.parents('.video-slider').find('.is-active').prev().removeClass('is-active');
  ...
  videoFrame.on('click', function () {
    var frame = $(this);
    frame.parents('.video-slider').find('.is-active').removeClass('is-active');
    frame.parent().addClass('is-active');
    videoButtons(frame);
    videoEmbed(frame.find('iframe'), frame.find('img'));
    videoPrev(frame);
    videoNext(frame);
  });
  setInterval(function () {
    if ($('.js-modal.is-open').length != 0) return;
    videoModal.find('iframe').remove();
    videoModal.find('img').remove();
  }, 1);
});
